<script lang="ts">
  ...
  type SourceType = 'local' | 'online' | null
  ...
  onMount(async () => {
    ...
    if (lastSource === 'local' || lastSource === 'online') {
      source = lastSource
    }
  ...
  let importedCount = $derived(importProgress?.importedElements ?? importProgress?.tilesImported ?? selectedCount)
  ...
  <aside class="w-96 bg-neutral-100 dark:bg-neutral-800 border-l border-neutral-200 dark:border-neutral-700 p-6 overflow-y-auto">
    {#if step === 'source'}
      ...
      <p class="text-sm text-neutral-500 dark:text-neutral-400">
        {$strings['Import.SourceIntro'] ?? 'Select a source to begin importing PLATEAU context data into your Revit project.'}
      </p>
    ...
    {:else if step === 'preflight'}
      <div class="flex items-center gap-2 mb-4">
        ...
        <h2 class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">
          {source === 'local' ? $strings['Import.Step.Preflight'] ?? 'Local Folder' : $strings['Import.Source.OnlineApi.Title'] ?? 'PLATEAU Online / API'}
        </h2>
      </div>
    ...
    {:else if step === 'import'}
      ...
      <div class="bg-white border border-neutral-200 dark:bg-neutral-900 dark:border-neutral-700 rounded-lg p-4">
        {#if importing}
          <div class="flex items-center gap-2 mb-3">
            <div class="w-4 h-4 border-2 border-neutral-600 border-t-teal-500 rounded-full animate-spin"></div>
            <span class="text-sm text-neutral-700 dark:text-neutral-300">{$strings['Import.ImportingTiles'] ?? 'Importing tiles...'}</span>
          </div>
          {#if importProgress}
            <div class="space-y-2">
