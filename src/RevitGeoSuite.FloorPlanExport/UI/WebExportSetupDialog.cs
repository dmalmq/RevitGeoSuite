...
internal sealed class WebExportSetupDialog : IDisposable
{
    ...
    {
        bridge.RegisterHandler(new DialogOpenFolderHandler(this));
        ...
        bridge.RegisterHandler(new SubmitHandler(this));
        bridge.RegisterHandler(new CancelHandler(this));
    }

    private ExporterInitialStateResponse BuildInitialState(ExporterSettingsPayload? payload = null)
    {
    ...
    private PreviewInitialStateResponse PreparePreview(ExporterSettingsPayload payload)
    {
    ...
    private PreviewInitialStateResponse GetPreviewInitialState()
    {
        return _previewState ?? PreparePreview(_currentPayload);
    }

    private PreviewViewPayload LoadPreviewView(long viewId)
    {
    ...
    private PreviewViewPayload AssignPreviewCategory(PreviewAssignmentRequest request)
    {
        ExportPreviewController controller = EnsurePreviewController();
    ...
    private PreviewViewPayload ClearPreviewAssignment(PreviewClearAssignmentRequest request)
    {
        ExportPreviewController controller = EnsurePreviewController();
        IReadOnlyList<string> floorTypeNames = WebPreviewPayloadMapper.CleanFloorTypeNames(request.FloorTypeNames);
    ...
    private PreviewViewPayload SavePreviewAssignments()
    {
        ExportPreviewController controller = EnsurePreviewController();
    ...
    private PreviewViewPayload DiscardPreviewAssignments()
    {
        ExportPreviewController controller = EnsurePreviewController();
    ...
    private ExporterRunResponse RunExport(ExporterSettingsPayload payload)
    {
        string? validationError = Validate(payload);
        if (validationError != null)
        {
    ...
    private ExecutionProgressInitialStateResponse BuildProgressInitialState()
    {
    ...
    private ExportResultInitialStateResponse BuildResultInitialState()
    {
    ...
    private ExecutionActionResponse OpenLastOutputDirectory()
    {
        UiLanguage language = ToSettings(_currentPayload).UiLanguage;
    ...
    private ExporterInitialStateResponse SaveProfile(ExporterSaveProfileRequest request)
    {
    ...
    private ExporterInitialStateResponse DeleteProfile(ExporterDeleteProfileRequest request)
    {
        ExportProfileScope scope = FromScopeString(request.Scope);
        string name = request.Name?.Trim() ?? string.Empty;
        ExportProfile? profile = _profiles.FirstOrDefault(candidate =>
    ...
    private ExporterSubmitResponse Submit(ExporterSettingsPayload payload)
    {
        string? validationError = Validate(payload);
        if (validationError != null)
        {
            return new ExporterSubmitResponse { Accepted = false, Error = validationError };
        }

        _currentPayload = ClonePayload(payload);
        Result = ToResult(_currentPayload);
        _window.Dispatcher.BeginInvoke(new Action(_window.Close));
        return new ExporterSubmitResponse { Accepted = true };
    }
    ...
    private sealed class DialogOpenFolderHandler : IRpcHandler
    {
        private readonly WebExportSetupDialog _dialog;

        public DialogOpenFolderHandler(WebExportSetupDialog dialog)
        {
            _dialog = dialog;
        }
        ...
        {
            DialogOpenFolderRequest request = PayloadReader.Read<DialogOpenFolderRequest>(payload);
            using WinForms.FolderBrowserDialog folderDialog = new()
            {
                Description = string.IsNullOrWhiteSpace(request.Title)
                    ? "Select output folder"
                    : request.Title,
                SelectedPath = request.InitialPath ?? string.Empty,
                ShowNewFolderButton = true,
            };

            WinForms.DialogResult result = _dialog.TryGetOwnerWindow() is WinForms.IWin32Window owner
                ? folderDialog.ShowDialog(owner)
                : folderDialog.ShowDialog();
            return Task.FromResult<object?>(new DialogOpenFolderResponse
            {
                Path = result == WinForms.DialogResult.OK ? folderDialog.SelectedPath : null,
            });
        }
    }
    ...
    private sealed class GetInitialStateHandler : IRpcHandler
    {
        ...
        public GetInitialStateHandler(WebExportSetupDialog dialog) => _dialog = dialog;

        public string Method => "exporter.getInitialState";
    ...
    private sealed class PreparePreviewHandler : IRpcHandler
    {
        ...
        public PreparePreviewHandler(WebExportSetupDialog dialog) => _dialog = dialog;

        public string Method => "exporter.preparePreview";
    ...
    private sealed class PreviewGetInitialStateHandler : IRpcHandler
    {
        ...
        public PreviewGetInitialStateHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class PreviewLoadViewHandler : IRpcHandler
    {
        ...
        public PreviewLoadViewHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class PreviewAssignCategoryHandler : IRpcHandler
    {
        ...
        public PreviewAssignCategoryHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class PreviewClearAssignmentHandler : IRpcHandler
    {
        ...
        public PreviewClearAssignmentHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class PreviewSaveAssignmentsHandler : IRpcHandler
    {
        ...
        public PreviewSaveAssignmentsHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class PreviewDiscardAssignmentsHandler : IRpcHandler
    {
        ...
        public PreviewDiscardAssignmentsHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class RunExportHandler : IRpcHandler
    {
        ...
        public RunExportHandler(WebExportSetupDialog dialog) => _dialog = dialog;

        public string Method => "exporter.run";
    ...
    private sealed class ProgressInitialStateHandler : IRpcHandler
    {
        ...
        public ProgressInitialStateHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class ResultInitialStateHandler : IRpcHandler
    {
        ...
        public ResultInitialStateHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class OpenOutputFolderHandler : IRpcHandler
    {
        ...
        public OpenOutputFolderHandler(WebExportSetupDialog dialog) => _dialog = dialog;
    ...
    private sealed class SaveProfileHandler : IRpcHandler
    {
        ...
        public SaveProfileHandler(WebExportSetupDialog dialog) => _dialog = dialog;

        public string Method => "exporter.saveProfile";
    ...
    private sealed class DeleteProfileHandler : IRpcHandler
    {
        ...
        public DeleteProfileHandler(WebExportSetupDialog dialog) => _dialog = dialog;

        public string Method => "exporter.deleteProfile";

        public Task<object?> HandleAsync(object? payload) =>
            Task.FromResult<object?>(_dialog.DeleteProfile(PayloadReader.Read<ExporterDeleteProfileRequest>(payload)));
    }

    private sealed class SubmitHandler : IRpcHandler
    {
        private readonly WebExportSetupDialog _dialog;

        public SubmitHandler(WebExportSetupDialog dialog) => _dialog = dialog;

        public string Method => "exporter.submit";

        public Task<object?> HandleAsync(object? payload) =>
            Task.FromResult<object?>(_dialog.Submit(PayloadReader.Read<ExporterSettingsPayload>(payload)));
    }
    ...
    private sealed class CancelHandler : IRpcHandler
    {
        ...
        public CancelHandler(WebExportSetupDialog dialog) => _dialog = dialog;

        public string Method => "exporter.cancel";
